package Catapulse::Formatter::Wiki;

use parent qw/Catapulse::Formatter/;

use URI;
use Scalar::Util qw/blessed/;
use Catapulse::Formatter::TOC;

=head1 NAME

Catapulse::Formatter::Wiki - Handle interpage linking.
...
sub format_link {
    ...
    # May as well smoke the page stash from Catapulse.pm since we got it eh?
    ...
    $c = Catapulse->context unless ref $c;
    ...
    for ($wikilink) {
        ...
        # Catapulse doesn't support periods in wikilinks because they conflict with actions ('.edit', '.info' etc.);
    ...
    # if the fragment was not properly formatted as a fragment (per the rules explained in Catapulse::Formatter::TOC::assembleAnchorName, i.e. i has an invalid character), convert it, unless it contains escaped characters already (.[0-9A-F]{2})
    if(Catapulse::Formatter::TOC->module_loaded){
        $fragment = Catapulse::Formatter::TOC::assembleAnchorName(undef, undef, undef, undef, $fragment)
    ...
    if (
        ...
        # drop spaces
        ref $c->stash->{page} eq 'Catapulse::Model::DBIC::Page' && $wikilink !~ m|^/|
        )
    {
    ...
    if ( defined $page and ref($page) ) {
      ...
      #   . qq{" href="$url/?edit=1">$formatted?</a></span>};
      return qq{<span class="newWikiWord"><a href="$url">$formatted?</a></span>};
    }
