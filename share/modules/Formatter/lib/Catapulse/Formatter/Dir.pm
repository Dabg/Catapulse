package Catapulse::Formatter::Dir;

use strict;
use warnings;
use base qw/Catapulse::Formatter/;
use Path::Class ();
use Catapulse::Formatter::File::Image;

my $debug=0;

=head1 NAME

Catapulse::Formatter::Dir - format local directory as XHTML
...
sub to_xhtml{
  ...
  if ( $files[0] ){
    ...
    foreach my $f (@files){
      ...
      $f =~ s/\./_/
        if ( ! Catapulse::Formatter::File::Image->can_format($1) );
