package Catapulse::Formatter::Pod;
...
Catapulse::Formatter::Pod - format part of content as POD
...
sub format_content {
    ...
    foreach my $line (@lines) {
        if ($pod) {
            if ( $line =~ m/^(.*)$end_re(.*)$/ ) {
                $$content .= Catapulse::Formatter::Pod->to_pod( $pod.$1, $c->req->base ).$2;
                $pod = "";
            }
            else { $pod .= $line . "\n"; }
        }
...
sub to_pod {
    my ( $class, $pod, $base ) = @_;
    my $result;
    my $parser = Catapulse::Formatter::Pod::Simple::HTML->new($base);
...
package Catapulse::Formatter::Pod::Simple::HTML;
...
L<Catapulse>, L<Module::Pluggable::Ordered>, L<POD::Tree::HTML>
